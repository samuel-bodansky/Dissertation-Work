...
\begin{theorem}
 Define $K=\Q(\sqrt{22268})$, $L=split(x^6-10x^4-7x^3+15x^2+14x+3,\Q[x])$. Then $K_1=\Q(\sqrt{76},\sqrt{293})$. Define $M=LK_1$. Then $M=K^{ur}$, as shown in the following diagram:   
\end{theorem}
\begin{center}
    ...
    \draw [-] (L) to node [pos=0.1,right,above]{$V_4$} (M);
      \end{tikzpicture}
\end{center}
Now define $K=\Q(\sqrt{-1567})$ and 
\begin{equation}
    L=split(x^9-2x^8+10x^7-25x^6+34x^5-40x^4+52x^3-45x^2+20x-4,\Q[x])
\end{equation}Then the following theorem is used:
\begin{theorem}
    Write $G=PSL_2(\mathbb{F}_8)$. $L$ is a G-extension of $\Q$; $1567$ is the only prime in this field which is ramified with index $2$. Abhyankar's Lemma shows that $LK/K$ is unramified at every prime. $G$ is a non-abelian simple group and so $L\cap K_1=\Q$. Therefore $\Gamma(LK_1/K_1)\cong \Gamma(L/\Q)\cong G$. 
\end{theorem}
