...
\end{equation}
It thereford holds that $delta_m \in \mathcal{M}(k_m)$ iff $g_m(x)$ is irreducible over $\q$. Now both the $\{F_n\}$ and $\{L_n\}$ have periods of length 50 modulo 151. For $m\equiv12 \: mod \: 25$ then 
$g_m(x) \equiv x^5 − 10x^3 − 20x^2 + 65x + 28\:(mod \:151)$ which is irreducible over $\q$. Let $\theta$ be a root of this polynomial. By a result by Sase, no primes are totally ramified over $\q(\theta)$. Therefore $E/k_m$ is an unramified cyclic quintic extension. 
\end{proof}
...
\cite{YAM2} defines the structure of $\Gamma(K_{ur}/K)$ where K is an imaginary quadratic number field with small conductors. Note that for all number fields with conductor $<1000$, $K_{ur}$ is one of $K_1$, $K_2$ and $K_3$. 
