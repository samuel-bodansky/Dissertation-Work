...
\cite{YAM2} defines the structure of $\Gamma(K_{ur}/K)$ where K is an imaginary quadratic number field with small conductors. Note that for all number fields with conductor $<1000$, $K_{ur}$ is one of $K_1$, $K_2$ and $K_3$. \par 
...
\par
$\mathbb{Q}(\sqrt{-1507})$ is the first imaginary quadratic field with an unramified $A_5$-extension over $\mathbb{Q}$. In fact, defining $L=split\lb x^5-5x^3+5x^2+24x+5,\q\rb$ gives such an $A_5$ extension of K. 
\begin{lemma}
...
In \cite{YAM2}, we have the following theorem:
\begin{theorem}
    ...
    \end{enumerate}
\end{theorem}
To prove this theorem, Yamamura gives five conditions for a quintic polynomial of the form \begin{equation}
    f(x)=x^5-2ax^3+bx+c, a,b,c \in \mathbb{Z}
\end{equation}. \begin{lemma}
Suppose f is as above and the five conditions hold. Then $\q(\sqrt{\Delta(f)})$ is a real quadratic number field and its splitting field is a strictly unramified $A_5$-extension. 
\end{lemma}
\begin{example}
    Take a=6,b=-3,c=-7. 
\end{example}
...
and $rd_{K} = 46.10$. Now h(K)=7. Define $g(x) = x^7-x^6-15x^5+20x^4+33x^3-22x^2-32x+8$. We can calculate that $K_1=split($g,\mathbb{Q}) with field discriminant $313^4$. Since $h(K_1) = 1$ and $rd_K = 46.10 < 56.47$ and 56.47 is the lower
...
\bibitem{KISH}Kishi, Yasuhiro. (2008). A new family of imaginary quadratic fields whose class number is divisible by five. Journal of Number Theory - J NUMBER THEOR. 128. 2450-2458. 10.1016/j.jnt.2008.02.016. 
\bibitem{WONG}Wong, Ka Lun, "Maximal Unramified Extensions of Cyclic Cubic Fields" (2011). All Theses and Dissertations. 2781. 
\bibitem{YAM2}
YAMAMURA, Ken. On infinite unramified Galois extensions of algebraic number fields with many primes decomposing almost completely. J. Math. Soc. Japan 38 (1986), no. 4, 599--605
\end{thebibliography}
